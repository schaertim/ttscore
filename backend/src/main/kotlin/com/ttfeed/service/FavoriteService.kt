...
import com.ttfeed.database.Favorites
import com.ttfeed.database.PlayerElos
...
import com.ttfeed.model.FavoriteCheckResponse
import com.ttfeed.model.FavoriteResponse
...
object FavoriteService {
    ...
    suspend fun getFavorites(userId: String): List<FavoriteResponse> = dbQuery {
        Favorites.selectAll()
    ...
    suspend fun check(userId: String, targetType: FollowTargetType, targetId: UUID): FavoriteCheckResponse =
        dbQuery {
            val row = Favorites.selectAll()
                .where {
     ...
     */
    suspend fun favorite(userId: String, targetType: FollowTargetType, targetId: UUID): FavoriteResponse? =
        dbQuery {
     ...
     */
    suspend fun getFavoritePlayers(userId: String): List<PlayerResponse> = dbQuery {
        val playerIds = Favorites.selectAll()
            .where {
                (Favorites.userId eq userId) and
    ...
    suspend fun unfavorite(userId: String, favoriteId: UUID): Boolean = dbQuery {
