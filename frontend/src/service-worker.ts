...
self.addEventListener('push', (event) => {
	...
	event.waitUntil(
		self.registration.showNotification(title, {
			body,
			// Not favicon.svg: its light/dark variants are toggled via a CSS media query that
			// Android's notification icon decoder ignores, rendering both stacked in one image.
			icon: '/icon-192.png',
			data: { url: url ?? '/' }
		})
	);
