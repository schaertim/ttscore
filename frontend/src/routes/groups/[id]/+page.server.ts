...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	if (session) {
		...
		const [favRes, followRes] = await Promise.allSettled([
			ktor.get(`/favorites/check?targetType=division_group&targetId=${id}`),
			ktor.get(`/follows/check?targetType=division_group&targetId=${id}`)
		]);
	...
	return { group, standings, matches: matches ?? [], favorited, favoriteId, notifying, notifyId };
};
